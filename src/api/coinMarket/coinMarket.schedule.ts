...
import { ASDkrwclosePrice, ASDkrwopenPrice, ASDusdclosePrice, ASDusdopenPrice, generateASDTokenPrice } from '../../bulkdata/tokens/bulkdataRow';
...
export const PriceUpdate = async () => {
        ...
        for (let symbol of symbols) {
            const [symbolRecord, created] = await TokenValue.findOrCreate({
                ...
                defaults: { 
                    ...
                    lastUppdateDate: resultUSD.data.data[symbol].quote.USD.last_updated.toString().slice(0,10),
                },
            });

            if (!created) { 
                symbolRecord.dailyEndPriceUSD = resultUSD.data.data[symbol].quote.USD.price;
                symbolRecord.dailyEndPriceKRW = resultKRW.data.data[symbol].quote.KRW.price;
                symbolRecord.lastUppdateDate = resultUSD.data.data[symbol].quote.USD.last_updated.toString().slice(0,10);
    ...
    cron.schedule('04 00 00 * * *', async () => {
        ...
        for (let symbol of symbols){
            ...
            if (!created) { 
                symbolRecord.dailyOpenPriceUSD = resultUSD.data.data[symbol].quote.USD.price;
                symbolRecord.dailyOpenPriceKRW = resultKRW.data.data[symbol].quote.KRW.price;
                symbolRecord.lastUppdateDate = resultUSD.data.data[symbol].quote.USD.last_updated.toString().slice(0,10);
                await symbolRecord.save();
            }
        }
